...
```
在线程池中主要有以下的参数:
  任务队列: 当创建了一个线程池后, 如果需要使用线程来执行任务, 那么我们就需要将这个任务(一般用
           Runnable接口实现类)提交到线程池中的任务队列, 这个任务队列可以是一个链表, 可以采用FIFO
           模式先进先出(先进先执行)来将任务分配给空闲的线程
  
  拒绝策略: 假设我们的线程为10个, 而我们提交了大量的任务, 比如说1000个, 可能会导致执行不过来, 所以
            需要这个拒绝策略来决定是否拒绝一些任务, 以及以什么方式去拒绝, 丢弃?抛出异常?

  min, active, max: 三者的大小表示当时线程池中的线程个数, 通过调整线程的个数为这些值可以动态的改变
                    线程池中线程的个数, 从而不会使得资源的浪费

线程池的一般实现思路: 我们在初始的时候创建一系列的线程在线程池中, 并使得这些线程持续运行, 即在run方
                    法中通过while的方式来使得run方法不会执行结束, 然后在while中执行任务, 从任务
                    队列中拿取任务, 当任务执行完毕后再换取下一个任务, 如果没有任务执行, 则当前线程
                    进入阻塞状态
```

> Notes: 实现了线程池中的线程依次执行线程任务, 以及拒绝策略, 终止线程池
